###########################################################################################
### PROJECT: Immune Repertoire. Analysis B cells antibodies for pregnancy, j_segment usage
###
### CITATION: 
###
### PROCESS: 
...
plot_directory <- "BCR/Results/J_segment_usage/"
...
data_qc_clones<-data_qc[,c("sample_label", "isotype", "V_J_lenghCDR3_Clone_igh", "j_segment", "IGHM_mutated", "IGHD_mutated")]
...
for (isotype in isotypes){
  ...
  ##copy pasted from VGeneUsageBCR.R, replaced data_qc_iso$v_gene with data_qc_iso$j_segment
  v_gene_counts<-as.data.frame.matrix(table(data_qc_iso$j_segment, data_qc_iso$sample_label))
  ...
  tiff(paste(plot_directory_iso, "barplot_jSegment_", isotype, ".tiff", sep = ""),res=300,w=4000,h=2000)
  p <- ggplot(v_gene_percents_summary_2, aes(x = gene, y = mean, fill = group)) +
    ...
    labs(title = paste("Pooled j segment usage - ", isotype, sep = "")) +
    xlab("J Segment") +
    ylab("% of clones") +
    coord_cartesian(ylim = c(0,1.1*max(0, v_gene_percents_summary_2$mean, v_gene_percents_summary_2$mean + v_gene_percents_summary_2$std,
  ...
  tiff(paste(plot_directory_iso, "barplot_jSegment_sig_", isotype, ".tiff", sep = ""),res=300,w=4000,h=2000)
  p <- ggplot(v_gene_percents_summary_2_sig, aes(x = gene, y = mean, fill = group)) +
    geom_bar(position = position_dodge(), stat = "identity", color = "black") +
    geom_errorbar(aes(ymin = mean - std, ymax = mean + std), width= 0.2, position = position_dodge(0.9)) +
    theme(text = element_text(size = 15),
          axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
    labs(title = paste("Significantly different j segments - ", isotype, sep = "")) + 
    xlab("J Segment") +
    ylab("% of clones") +
  ...
  ##heatmaps
  print(paste(isotype, "heatmap", sep = ", "))
  ...
  tiff(paste(plot_directory_iso, "heatmap_jSegment.tiff", sep = ""),res=300,w=2000,h=2000)
  p <- pheatmap(v_gene_percents, annotation_col = my_sample_col,
                color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                main = paste("J Segment Heatmap - ", isotype), fontsize = 8, fontsize_row = 6)
  print(p)
  dev.off()
  
  tiff(paste(plot_directory_iso, "heatmap_jSegment_usage_cutoff.tiff", sep = ""),res=300,w=2000,h=2000)
  ...
  tiff(paste(plot_directory_iso, "heatmap_jSegment_correlation.tiff", sep = ""),res=300,w=2000,h=2000)
  p <- pheatmap(cor(v_gene_percents), annotation_col = my_sample_col,
                color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                main = paste("J Segment Usage Sample Correlation - ", isotype), fontsize = 8, fontsize_row = 6)
...
for (isotype in c("IGHD", "IGHM")){
  ...
  for (mutate in c("unmutated", "mutated")){
    ...
    p <- ggplot(v_gene_percents_summary_2, aes(x = reorder(gene, gene_order), y = mean, fill = group)) +
      ...
      theme(text = element_text(size = 15),
            axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
      labs(title = paste("Pooled j segment usage - ", isotype, " ", mutate, sep = "")) +
      xlab("J segment") +
      ylab("% of clones") +
      coord_cartesian(ylim = c(0,1.1*max(0, v_gene_percents_summary_2$mean + v_gene_percents_summary_2$std,
                                         na.rm = TRUE)), #0 provided in max as a fail case for empty dataframes
