...
summary_data_TCR <- summary_data
summary_data_BCR <- read.csv('Stanford_2/BCR/summary_data.csv') #using to extract outcome info
summary_data <- summary_data_BCR[,c("X", "sample", "Outcome")]
summary_data$row <- summary_data$X
rownames(summary_data) <- summary_data$X

directory <- "Stanford_2/TCR/"
plot_directory <- "Stanford_2/TCR/Results/V_gene_usage/"
dir.create(plot_directory)

##Isolate unique clones per isotype per sample to perform clone-level analysis
data_qc<-data_qc[,c("sample_label", "V_J_length_CDR3_Clone_tcrb", "v_gene")]
#rm(data_qc) #frees up memory just in case

#mask<-!duplicated(data_qc_clones[,c("V_J_length_CDR3_Clone_tcrb", "sample_label", "isotype")])
...
for (sample in samples){
  ...
  mask<-!duplicated(data_qc_temp[,c("V_J_length_CDR3_Clone_tcrb", "sample_label")])
...
save(data_qc_clones, file = 'Stanford_2/TCR/TCR_unique_clones.RData')
...
v_gene_counts<-as.data.frame.matrix(table(data_qc_clones$v_gene, data_qc_clones$sample))
...
for (type in sample_types){
  ...
  tiff(paste(plot_directory, type, ".tiff", sep = ""),res=300,w=4000,h=2000)
  p <- ggplot(v_gene_percents_summary_2, aes(x = gene, y = mean, fill = group)) +
    ...
    labs(title = paste0("TCR - ", type, " - pooled v gene usage by outcome")) +
    xlab("V Gene") +
    ylab("% of clones") +
  ...
  if (nrow(v_gene_percents_summary_2_sig) > 0){
    tiff(paste(plot_directory, type, "_sig.tiff", sep = ""),res=300,w=4000,h=2000)
    p <- ggplot(v_gene_percents_summary_2_sig, aes(x = gene, y = mean, fill = group)) +
      geom_bar(position = position_dodge(), stat = "identity", color = "black") +
      geom_errorbar(aes(ymin = mean - std, ymax = mean + std), width= 0.2, position = position_dodge(0.9)) +
      theme(text = element_text(size = 15),
            axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
      labs(title = paste("Significantly different genes - ", type, sep = "")) + 
      xlab("V Gene") +
      ylab("% of clones") +
      coord_cartesian(ylim = c(0,1.1*max(0, v_gene_percents_summary_2$mean + v_gene_percents_summary_2$std,
                                         na.rm = TRUE)),
  ...
  tiff(paste0(plot_directory, type, "heatmap.tiff"),res=300,w=2000,h=2000)
  p <- pheatmap(v_gene_percents_combined, annotation_col = col_labels,
                ...
                main = paste0("Heatmap of Genes/Samples - TCR - ", type), fontsize = 8, fontsize_row = 6)
  print(p)
  dev.off()
  
  #heatmap of sig genes only
  if(nrow(v_gene_percents_combined[sig_genes,]) > 1){
    tiff(paste0(plot_directory, type, "heatmap_sig_genes.tiff"),res=300,w=2000,h=2000)
    p <- pheatmap(v_gene_percents_combined[sig_genes,], annotation_col = col_labels,
                  annotation_colors = ann_colors,
                  color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                  main = paste0("Heatmap of Significant Genes - TCR - ", type), fontsize = 8, fontsize_row = 6)
    print(p)
    dev.off()
  }
}
...
for (outcome in sample_outcomes){
  ...
  tiff(paste(plot_directory, outcome, ".tiff", sep = ""),res=300,w=4000,h=2000)
  p <- ggplot(v_gene_percents_summary_2, aes(x = gene, y = mean, fill = group)) +
    geom_bar(position = position_dodge(), stat = "identity", color = "black") +
    geom_errorbar(aes(ymin = mean - std, ymax = mean + std), width= 0.2, position = position_dodge(0.9)) +
    theme(text = element_text(size = 15),
          axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
    labs(title = paste0("TCR - ", outcome, " - pooled v gene usage by sample type")) +
    xlab("V Gene") +
    ylab("% of clones") +
  ...
  if (nrow(v_gene_percents_summary_2_sig) > 0){
    tiff(paste(plot_directory, outcome, "_sig.tiff", sep = ""),res=300,w=4000,h=2000)
    p <- ggplot(v_gene_percents_summary_2_sig, aes(x = gene, y = mean, fill = group)) +
      geom_bar(position = position_dodge(), stat = "identity", color = "black") +
      geom_errorbar(aes(ymin = mean - std, ymax = mean + std), width= 0.2, position = position_dodge(0.9)) +
      theme(text = element_text(size = 15),
            axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
      labs(title = paste("Significantly different genes - ", outcome, sep = "")) + 
      xlab("V Gene") +
      ylab("% of clones") +
      coord_cartesian(ylim = c(0,1.1*max(0, v_gene_percents_summary_2$mean + v_gene_percents_summary_2$std,
                                         na.rm = TRUE)),
  ...
  tiff(paste0(plot_directory, outcome, "heatmap.tiff"),res=300,w=2000,h=2000)
  p <- pheatmap(v_gene_percents_combined, annotation_col = col_labels,
                annotation_colors = ann_colors,
                color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                main = paste0("Heatmap of Genes/Samples - TCR - ", outcome), fontsize = 8, fontsize_row = 6)
  print(p)
  dev.off()
  
  #heatmap of sig genes only
  if(nrow(v_gene_percents_combined[sig_genes,]) > 1){
    tiff(paste0(plot_directory, outcome, "heatmap_sig_genes.tiff"),res=300,w=2000,h=2000)
    p <- pheatmap(v_gene_percents_combined[sig_genes,], annotation_col = col_labels,
                  annotation_colors = ann_colors,
                  color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                  main = paste0("Heatmap of Significant Genes - TCR - ", outcome), fontsize = 8, fontsize_row = 6)
    print(p)
    dev.off()
  }
  
  if (outcome == 'Term'){
    write.csv(v_gene_percents, file = paste0(plot_directory, "Term_v_gene_percents.csv"))
  }
}
