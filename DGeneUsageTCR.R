###########################################################################################
### PROJECT: Immune Repertoire. Analysis T cells antibodies for pregnancy, d_gene usage
###
### CITATION: 
###
### PROCESS: 
...
plot_directory <- "TCR/Results/D_gene_usage/"
dir.create(plot_directory)

##Isolate unique clones per sample to perform clone-level analysis
data_qc_clones<-data_qc[,c("sample_label", "V_J_length_CDR3_Clone_tcrb", "d_gene")]
...
##c/p from VGeneUsageTCR.R, data_qc_clones$v_gene --> data_qc_clones$d_gene
v_gene_counts<-as.data.frame.matrix(table(data_qc_clones$d_gene, data_qc_clones$sample_label))
...
tiff(paste(plot_directory, "barplot_dGene_usage_TCR.tiff", sep = ""),res=300,w=4000,h=2000)
ggplot(v_gene_percents_summary_2, aes(x = gene, y = mean, fill = group)) +
  ...
  labs(title = "Pooled d gene usage - TCR") +
  xlab("D Gene") +
  ylab("% of clones") +
...
tiff(paste(plot_directory, "barplot_dGene_usage_sig_TCR.tiff", sep = ""),res=300,w=4000,h=2000)
...
tiff(paste(plot_directory, "heatmap_dGene.tiff", sep = ""),res=300,w=2000,h=2000)
pheatmap(v_gene_percents, annotation_col = my_sample_col,
         color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
         main = "D Gene Usage Heatmap - TCR", fontsize = 8, fontsize_row = 6)
dev.off()

##heatmaps of just significant genes
tiff(paste(plot_directory, "heatmap_dGene_sig.tiff", sep = ""),res=300,w=2000,h=2000)
pheatmap(v_gene_percents[sig_genes,], annotation_col = my_sample_col,
         color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
         main = "D Gene Usage Heatmap (Significant) - TCR", fontsize = 8, fontsize_row = 6)
dev.off()


#heatmap, exclude genes with consistent low usage (mean: 0.005)
tiff(paste(plot_directory, "heatmap_dGene_usage_cutoff.tiff", sep = ""),res=300,w=2000,h=2000)
p <- pheatmap(v_gene_percents[(rowSums(v_gene_percents)/ncol(v_gene_percents)) > 0.005,], annotation_col = my_sample_col,
              color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
              main = paste("D Gene Usage Heatmap - TCR"), fontsize = 8, fontsize_row = 6)
print(p)
dev.off()

##correlatin matrix heatmap
tiff(paste(plot_directory, "heatmap_dGene_correlation.tiff", sep = ""),res=300,w=2000,h=2000)
p <- pheatmap(cor(v_gene_percents), annotation_col = my_sample_col,
              color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
              main = paste("D Gene Usage Sample Correlation - TCR"), fontsize = 8, fontsize_row = 6)
