...
def ProcessSample(data_clonesInference_sample_unique):
    result = pd.DataFrame([])
    #min_read_depth = min(pd.Series(data_clonesInference['sample_label']).value_counts())
    min_read_depth = 291635
...
data_clonesInference = pd.read_csv("/Users/Pinedasans/ImmuneRep_Pregnancy/Data/BCR/data_clonesInference.txt",sep="\t")

###Obtain the unique subjects
sample_unique = data_clonesInference['sample_label'].unique()
...
for i in range(0,len(sample_unique)):
    print (i)
    data_clonesInference_sample_unique = data_clonesInference[data_clonesInference['sample_label'] == sample_unique[i]]
...
###Result    
result_ClonesInfered.to_csv('/Users/Pinedasans/ImmuneRep_Pregnancy/Data/BCR/ClonesInfered_downsampled.csv')
