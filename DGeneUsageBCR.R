###########################################################################################
### PROJECT: Immune Repertoire. Analysis B cells antibodies for pregnancy, d_gene usage
###
### CITATION: 
###
### PROCESS: 
...
plot_directory <- "BCR/Results/D_gene_usage/"
...
data_qc_clones<-data_qc[,c("sample_label", "isotype", "V_J_lenghCDR3_Clone_igh", "d_gene", "IGHM_mutated", "IGHD_mutated")]
...
for (isotype in isotypes){
  ...
  ##copy pasted from VGeneUsageBCR.R, replaced data_qc_iso$v_gene with data_qc_iso$d_gene
  v_gene_counts<-as.data.frame.matrix(table(data_qc_iso$d_gene, data_qc_iso$sample_label))
  ...
  tiff(paste(plot_directory_iso, "barplot_pooled_", isotype, ".tiff", sep = ""),res=300,w=4000,h=2000)
  p <- ggplot(v_gene_percents_summary_2, aes(x = gene, y = mean, fill = group)) +
    ...
    labs(title = paste("Pooled d gene usage - ", isotype, sep = "")) +
    xlab("D Gene") +
    ylab("% of clones") +
  ...
  tiff(paste(plot_directory_iso, "barplot_pooled_sig_", isotype, ".tiff", sep = ""),res=300,w=4000,h=2000)
  ...
  p <- pheatmap(v_gene_percents, annotation_col = my_sample_col,
                ...
                main = paste("D Gene Usage Heatmap - ", isotype), fontsize = 8, fontsize_row = 6)
  print(p)
  dev.off()
  
  tiff(paste(plot_directory_iso, "heatmap__usage_cutoff.tiff", sep = ""),res=300,w=2000,h=2000)
  ...
  p <- pheatmap(cor(v_gene_percents), annotation_col = my_sample_col,
                color = rev(colorRampPalette(rev(brewer.pal(n = 7, name = "YlOrRd")))(100)),
                main = paste("D Gene Usage Sample Correlation - ", isotype), fontsize = 8, fontsize_row = 6)
